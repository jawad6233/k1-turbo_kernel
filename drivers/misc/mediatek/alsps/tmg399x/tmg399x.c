...
#ifdef BUILD_CTS
	__ATTR(ges_mode_state, 0664, tmg399x_ges_mode_show,
			tmg399x_ges_mode_store),
	__ATTR(gesture, 0664, tmg399x_gesture_show,
...
/*********** register input device  **************/
	...
	ret = tmg399x_create_attr(&tmg399x_alsps_driver.driver);
	if (ret)
...
static int tmg399x_remove(struct i2c_client *client)
{
	...
	if (chip->p_idev) {
		tmg399x_delete_attr(&tmg399x_alsps_driver.driver);
		tmg399x_remove_sysfs_interfaces(&chip->p_idev->dev,
				prox_attrs, ARRAY_SIZE(prox_attrs));
		input_unregister_device(chip->p_idev);
	}
